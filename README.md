...
账户有 `expires_at`（或永久），面板提供三种调整方式：

| 操作 | 语义 |
|------|------|
| **续期 N 天** | 在 `max(now, 到期时间)` 上**叠加**；永久账户叠加无意义，**保持永久不变**并提示「该账户为永久有效，无需续期」 |
| **设置剩余 N 天** | **覆盖**为 now + N 天（忽略当前到期时间），可用于把永久账户改为限时 |
| **设为永久** | 置为永不过期；若账户此前因到期被暂停，同时解除暂停 |

- 到期后 `ExpiryScheduler`（60s 巡检）+ 端点守卫硬停用：停 Bot、断开房间、暂停插件（保留启用标记）
- 账户可自助用授权码**兑换**（叠加语义同上），过期账户兑换后自动恢复运行
- **永久账户兑换授权码会被拒绝，且不消耗该授权码**——避免用户白白损失一个码
