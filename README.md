...
v1.1.0 起，MisMiss 从「单服务器仪表盘」升级为**多账户面板**：一个面板可托管多个直播账户，每个账户拥有完全隔离的 Bot、直播间、插件副本与配置，并可通过子域名登录自己的独立门户。

## ✨ 特性

- 🖥️ **Web 控制台** — React + TypeScript + Tailwind CSS 现代化面板，完全替代 CLI
- 👥 **多账户体系** — 一个面板托管 N 个账户，实例/配置/权限/数据完全隔离；每个账户 = 1 个 Bot + 1 个直播间
- 🚪 **账户独立门户** — `user.<域名>` 子域名入口，直播主用自己的凭据登录，自助改密、自助兑换授权码（过期账户兑换后自动恢复），无任何面板功能
...
- **管理面板**：默认用户名密码均为 **MisMiss**，首次登录强制改密
- **账户门户**：访问 `http://user.localhost:15173`，用面板中创建的账户凭据登录
