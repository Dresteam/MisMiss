...
# Linux / macOS
bash scripts/docker-release.sh 1.1.0
```

产物：`dist/mismiss-1.1.0-docker.zip`（Windows）或 `dist/mismiss-1.1.0-docker.tar.gz`（Linux / macOS）。
...
cd /opt/mismiss && unzip mismiss-1.1.0-docker.zip && cd mismiss-1.1.0-docker
bash deploy.sh
```
...
bash scripts/docker-release.sh 1.1.0             # 构建镜像 + 打包 → dist/
powershell -File scripts\docker-release.ps1 -Version 1.1.0
