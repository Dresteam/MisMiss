...
class ExamplePlugin(Plugin):
    ...
    async def initialize(self) -> None:
        ...
        print("[ExamplePlugin] 示例插件初始化完成！")

    async def terminate(self) -> None:
        """插件终止 —— 在卸载/禁用前被调用。"""
        print("[ExamplePlugin] 示例插件已终止。")

    @event_handler
    def on_open(self, event: LiveOpenEvent) -> None:
        """开播事件。"""
        print(f"[ExamplePlugin] 🔴 直播间 {event.livestream.room_name} 开播了！")

    @event_handler
    def on_close(self, event: LiveCloseEvent) -> None:
        """下播事件。"""
        print(f"[ExamplePlugin] ⚫ 直播间 {event.livestream.room_name} 下播了。")
    ...
    def on_join(self, event: LiveJoinEvent) -> None:
        """用户进入事件。"""
        print(f"[ExamplePlugin] 👋 {event.user.name} 进入了直播间")
