...
from core.network.urls import parse_live_id
...
async def live_add(
    ...
    ``live_id`` 既可直接给数字，也可粘贴直播间链接
    （``https://fm.missevan.com/live/869198039``，带结尾斜杠或查询串都行）。
    """
    try:
        live_id = parse_live_id(req.live_id)
